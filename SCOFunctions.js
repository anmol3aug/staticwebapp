...
var ttaken ="";
var scr;
	function setBookmark(no)
	{
		slide = doLMSSetValue("cmi.core.lesson_location", no);
		slide = doLMSCommit();
		//alert(" SET SLIDE BOOKMARK >  " + no);
		return slide
	}
	function getBookmarkNo()
	{
		var lessonStatus=doLMSGetValue("cmi.core.lesson_status");
		slide = doLMSGetValue("cmi.core.lesson_location");
		
		if ((slide == 'null' || slide == '') && lessonStatus=='incomplete')
		{
			  slide = '0:0';
		}else if(lessonStatus=='passed'){
			  slide = "completed";
		}else{			 
             /* input_box=confirm("Click OK to Resume the course or Click CANCEL to Start course again");
			  if (input_box==true){ 
				alert(">> callSlideNo " + slide);
				return slide;		
			  }else
				{
				  slide='0:0';
		...
		//alert(" GET SLIDE BOOKMARK >  " + slide);
		return slide;
	}
	
//end scorm bookmarkinng-------------------------------------------------------------------------------------------------------

//certificate...................................................................................................................

	function getCertificateDetails(sname,cstatus,ttaken,scr)
	{
		sname=doLMSGetValue("cmi.core.student_name")
		cstatus=doLMSGetValue("cmi.core.lesson_status");
		ttaken=doLMSGetValue("cmi.core.session_time");
		scr=doLMSGetValue("cmi.core.score.raw");
		
		return sname+"*"+cstatus+"*"+ttaken+"*"+scr;
	}
...
{
	var result;
	//result = doLMSSetValue("cmi.core.lesson_status", "passed");
...
{
	var result;
	result = doLMSSetValue("cmi.core.lesson_status", "failed");
	result = doLMSCommit();
}
