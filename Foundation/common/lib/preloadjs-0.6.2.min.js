...
createjs.Elements=s}());(function(){var s={};s.ABSOLUTE_PATT=/^(?:\w+:)?\/{2}/i;s.RELATIVE_PATT=(/^[./]*?\//i);s.EXTENSION_PATT=/\/?[^/]+\.(\w{1,5})$/i;s.parseURI=function(path){var info={absolute:!1,relative:!1,protocol:null,hostname:null,port:null,pathname:null,search:null,hash:null,host:null};if(path==null){return info}
...
var match;if(s.ABSOLUTE_PATT.test(path)){info.absolute=!0}else if(s.RELATIVE_PATT.test(path)){info.relative=!0}
...
s.isImageTag=function(item){return item instanceof HTMLImageElement};s.isAudioTag=function(item){if(window.HTMLAudioElement){return item instanceof HTMLAudioElement}else{return!1}};s.isVideoTag=function(item){if(window.HTMLVideoElement){return item instanceof HTMLVideoElement}else{return!1}};createjs.DomUtils=s}());(function(){var s={};s.parseXML=function(text){var xml=null;try{if(window.DOMParser){var parser=new DOMParser();xml=parser.parseFromString(text,"text/xml")}}catch(e){}
if(!xml){try{xml=new ActiveXObject("Microsoft.XMLDOM");xml.async=!1;xml.loadXML(text)}catch(e){xml=null}}
return xml};s.parseJSON=function(value){if(value==null){return null}
try{return JSON.parse(value)}catch(e){throw e}};createjs.DataUtils=s}());this.createjs=this.createjs||{};(function(){var s={};s.BINARY="binary";s.CSS="css";s.FONT="font";s.FONTCSS="fontcss";s.IMAGE="image";s.JAVASCRIPT="javascript";s.JSON="json";s.JSONP="jsonp";s.MANIFEST="manifest";s.SOUND="sound";s.VIDEO="video";s.SPRITESHEET="spritesheet";s.SVG="svg";s.TEXT="text";s.XML="xml";createjs.Types=s}());this.createjs=this.createjs||{};(function(){var s={};s.POST="POST";s.GET="GET";createjs.Methods=s}());this.createjs=this.createjs||{};(function(){"use strict";function LoadItem(){this.src=null;this.type=null;this.id=null;this.maintainOrder=!1;this.callback=null;this.data=null;this.method=createjs.Methods.GET;this.values=null;this.headers=null;this.withCredentials=!1;this.mimeType=null;this.crossOrigin=null;this.loadTimeout=s.LOAD_TIMEOUT_DEFAULT};var p=LoadItem.prototype={};var s=LoadItem;s.LOAD_TIMEOUT_DEFAULT=8000;s.create=function(value){if(typeof value=="string"){var item=new LoadItem();item.src=value;return item}else if(value instanceof s){return value}else if(value instanceof Object&&value.src){if(value.loadTimeout==null){value.loadTimeout=s.LOAD_TIMEOUT_DEFAULT}
return value}else{throw new Error("Type not recognized.")}};p.set=function(props){for(var n in props){this[n]=props[n]}
return this};createjs.LoadItem=s}());(function(){var s={};s.isBinary=function(type){switch(type){case createjs.Types.IMAGE:case createjs.Types.BINARY:return!0;default:return!1}};s.isText=function(type){switch(type){case createjs.Types.TEXT:case createjs.Types.JSON:case createjs.Types.MANIFEST:case createjs.Types.XML:case createjs.Types.CSS:case createjs.Types.SVG:case createjs.Types.JAVASCRIPT:case createjs.Types.SPRITESHEET:return!0;default:return!1}};s.getTypeByExtension=function(extension){if(extension==null){return createjs.Types.TEXT}
switch(extension.toLowerCase()){case "jpeg":case "jpg":case "gif":case "png":case "webp":case "bmp":return createjs.Types.IMAGE;case "ogg":case "mp3":case "webm":return createjs.Types.SOUND;case "mp4":case "webm":case "ts":return createjs.Types.VIDEO;case "json":return createjs.Types.JSON;case "xml":return createjs.Types.XML;case "css":return createjs.Types.CSS;case "js":return createjs.Types.JAVASCRIPT;case 'svg':return createjs.Types.SVG;default:return createjs.Types.TEXT}};createjs.RequestUtils=s}());this.createjs=this.createjs||{};(function(){"use strict";function AbstractLoader(loadItem,preferXHR,type){this.EventDispatcher_constructor();this.loaded=!1;this.canceled=!1;this.progress=0;this.type=type;this.resultFormatter=null;if(loadItem){this._item=createjs.LoadItem.create(loadItem)}else{this._item=null}
...
this.loaded=!0;var event=new createjs.Event("complete");event.rawResult=this._rawResult;if(this._result!=null){event.result=this._result}
...
return this._response};p.cancel=function(){this.canceled=!0;this._clean();this._request.abort()};p.load=function(){if(this._request==null){this._handleError();return}
if(this._request.addEventListener!=null){this._request.addEventListener("loadstart",this._handleLoadStartProxy,!1);this._request.addEventListener("progress",this._handleProgressProxy,!1);this._request.addEventListener("abort",this._handleAbortProxy,!1);this._request.addEventListener("error",this._handleErrorProxy,!1);this._request.addEventListener("timeout",this._handleTimeoutProxy,!1);this._request.addEventListener("load",this._handleLoadProxy,!1);this._request.addEventListener("readystatechange",this._handleReadyStateChangeProxy,!1)}else{this._request.onloadstart=this._handleLoadStartProxy;this._request.onprogress=this._handleProgressProxy;this._request.onabort=this._handleAbortProxy;this._request.onerror=this._handleErrorProxy;this._request.ontimeout=this._handleTimeoutProxy;this._request.onload=this._handleLoadProxy;this._request.onreadystatechange=this._handleReadyStateChangeProxy}
...
this.loaded=!0;var error=this._checkError();if(error){this._handleError(error);return}
...
req.open(item.method||createjs.Methods.GET,src,!0);if(crossdomain&&req instanceof XMLHttpRequest&&this._xhrLevel==1){headers.Origin=location.origin}
...
if(r){this._disposeItem(this.getItem(item))}else{for(var i=this._currentLoads.length-1;i>=0;i--){var loadItem=this._currentLoads[i].getItem();if(loadItem.id==item||loadItem.src==item){this._currentLoads.splice(i,1)[0].cancel();itemsWereRemoved=!0;break}}}}
...
this.loadManifest(a,!1)};p.installPlugin=function(plugin){if(plugin==null){return}
...
this._addItem(file,null,basePath);if(loadNow!==!1){this.setPaused(!1)}else{this.setPaused(!0)}};p.loadManifest=function(manifest,loadNow,basePath){var fileList=null;var path=null;if(Array.isArray(manifest)){if(manifest.length==0){var event=new createjs.ErrorEvent("PRELOAD_MANIFEST_EMPTY");this._sendError(event);return}
...
if(loadNow!==!1){this.setPaused(!1)}else{this.setPaused(!0)}};p.load=function(){this.setPaused(!1)};p.getItem=function(value){return this._loadItemsById[value]||this._loadItemsBySrc[value]};p.getResult=function(value,rawResult){var item=this._loadItemsById[value]||this._loadItemsBySrc[value];if(item==null){return null}
var id=item.id;if(rawResult&&this._loadedRawResults[id]){return this._loadedRawResults[id]}
return this._loadedResults[id]};p.getItems=function(loaded){var arr=[];for(var n in this._loadItemsById){var item=this._loadItemsById[n];var result=this.getResult(n);if(loaded===!0&&result==null){continue}
arr.push({item:item,result:result,rawResult:this.getResult(n,!0)})}
...
this._scriptOrder.length=0;this._loadedScripts.length=0;this.loadStartWasDispatched=!1;this._itemCount=0;this._lastProgress=NaN};p._addItem=function(value,path,basePath){var item=this._createLoadItem(value,path,basePath);if(item==null){return}
...
var customHandler=this._typeCallbacks[item.type]||this._extensionCallbacks[item.ext];if(customHandler){var result=customHandler.callback.call(customHandler.scope,item,this);if(result===!1){return null}else if(result===!0){}else if(result!=null){item._loader=result}
...
if(!this._loadStartWasDispatched){this._sendLoadStart();this._loadStartWasDispatched=!0}
if(this._numItems==this._numItemsLoaded){this.loaded=!0;this._sendComplete();if(this.next&&this.next.load){this.next.load()}}else{this.loaded=!1}
...
for(var i=0;i<list.length;i++){var item=list[i].item;this._loadItemsBySrc[item.src]=item;this._loadItemsById[item.id]=item;this._loadedResults[item.id]=list[i].result;this._loadedRawResults[item.id]=list[i].rawResult}};p._finishOrderedItem=function(loader,loadFailed){var item=loader.getItem();if((this.maintainScriptOrder&&item.type==createjs.Types.JAVASCRIPT)||item.maintainOrder){if(loader instanceof createjs.JavaScriptLoader){this._currentlyLoadingScript=!1}
var index=createjs.indexOf(this._scriptOrder,item);if(index==-1){return!1}
this._loadedScripts[index]=(loadFailed===!0)?!0:item;this._checkScriptLoadOrder();return!0}
return!1};p._checkScriptLoadOrder=function(){var l=this._loadedScripts.length;for(var i=0;i<l;i++){var item=this._loadedScripts[i];if(item===null){break}
if(item===!0){continue}
var loadItem=this._loadedResults[item.id];if(item.type==createjs.Types.JAVASCRIPT){createjs.DomUtils.appendToHead(loadItem)}
var loader=item._loader;this._processFinishedLoad(item,loader);this._loadedScripts[i]=!0}};p._processFinishedLoad=function(item,loader){this._numItemsLoaded++;if(!this.maintainScriptOrder&&item.type==createjs.Types.JAVASCRIPT){var tag=loader.getTag();createjs.DomUtils.appendToHead(tag)}
this._updateProgress();this._sendFileComplete(item,loader);this._loadNext()};p._canStartLoad=function(loader){if(!this.maintainScriptOrder||loader.preferXHR){return!0}
var item=loader.getItem();if(item.type!=createjs.Types.JAVASCRIPT){return!0}
if(this._currentlyLoadingScript){return!1}
var index=this._scriptOrder.indexOf(item);var i=0;while(i<index){var checkItem=this._loadedScripts[i];if(checkItem==null){return!1}
i++}
this._currentlyLoadingScript=!0;return!0};p._removeLoadItem=function(loader){var l=this._currentLoads.length;for(var i=0;i<l;i++){if(this._currentLoads[i]==loader){this._currentLoads.splice(i,1);break}}};p._cleanLoadItem=function(loader){var item=loader.getItem();if(item){delete item._loader}}
...
this._loadTimeout=setTimeout(createjs.proxy(this._handleTimeout,this),this._item.loadTimeout);this.dispatchEvent("loadstart")};p._handleTimeout=function(){this._stopWatching();this.dispatchEvent(new createjs.ErrorEvent("PRELOAD_TIMEOUT"))};p._createRequest=function(){return this._request};p.handleEvent=function(event){switch(event.type){case "complete":this._rawResult=event.target._response;this._result=!0;this._parseCSS(this._rawResult);break;case "error":this._stopWatching();this.AbstractLoader_handleEvent(event);break}};p._watchCSS=function(){var src=this._item.src;if(src instanceof HTMLStyleElement){if(this._injectCSS&&!src.parentNode){(document.head||document.getElementsByTagName('head')[0]).appendChild(src)}
this._injectCSS=!1;src="\n"+src.textContent}
if(src.search(/\n|\r|@font-face/i)!==-1){this._parseCSS(src);return!0}
this._request=new createjs.XHRRequest(this._item);return!1};p._parseCSS=function(css){var regex=/@font-face\s*\{([^}]+)}/g
while(!0){var result=regex.exec(css);if(!result){break}
...
if(this._tag!=null){this._preferXHR=!1}else{this._tag=createjs.Elements.img()}
...
var crossOrigin=this._item.crossOrigin;if(crossOrigin==!0){crossOrigin="Anonymous"}
if(crossOrigin!=null&&!createjs.URLUtils.isLocal(this._item)){this._tag.crossOrigin=crossOrigin}
this.AbstractLoader_load()};p._updateXHR=function(event){event.loader.mimeType='text/plain; charset=x-user-defined-binary';if(event.loader.setResponseType){event.loader.setResponseType("blob")}};p._formatResult=function(loader){return this._formatImage};p._formatImage=function(successCallback,errorCallback){var tag=this._tag;var URL=window.URL||window.webkitURL;if(!this._preferXHR){}else if(URL){var objURL=URL.createObjectURL(this.getResult(!0));tag.src=objURL;tag.addEventListener("load",this._cleanUpURL,!1);tag.addEventListener("error",this._cleanUpURL,!1)}else{tag.src=this._item.src}
...
if(this._tag!=null){this._preferXHR=!1}};var p=createjs.extend(SoundLoader,createjs.AbstractMediaLoader);var s=SoundLoader;s.canLoadItem=function(item){return item.type==createjs.Types.SOUND};p._createTag=function(src){var tag=createjs.Elements.audio();tag.autoplay=!1;tag.preload="none";tag.src=src;return tag};createjs.SoundLoader=createjs.promote(SoundLoader,"AbstractMediaLoader")}());this.createjs=this.createjs||{};(function(){"use strict";function VideoLoader(loadItem,preferXHR){this.AbstractMediaLoader_constructor(loadItem,preferXHR,createjs.Types.VIDEO);if(createjs.DomUtils.isVideoTag(loadItem)||createjs.DomUtils.isVideoTag(loadItem.src)){this.setTag(createjs.DomUtils.isVideoTag(loadItem)?loadItem:loadItem.src);this._preferXHR=!1}else{this.setTag(this._createTag())}};var p=createjs.extend(VideoLoader,createjs.AbstractMediaLoader);var s=VideoLoader;p._createTag=function(){return createjs.Elements.video()};s.canLoadItem=function(item){return item.type==createjs.Types.VIDEO};createjs.VideoLoader=createjs.promote(VideoLoader,"AbstractMediaLoader")}());this.createjs=this.createjs||{};(function(){"use strict";function SpriteSheetLoader(loadItem,preferXHR){this.AbstractLoader_constructor(loadItem,preferXHR,createjs.Types.SPRITESHEET);this._manifestQueue=null}
