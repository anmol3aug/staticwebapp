...
eLearningApp.controller('multipleChoice', function($scope, $http, $document ) {   
    ...
    $http.get(__gjsonfilePath)
		...
		function playAudio(sndName){
			//audio-act 
			audioAct = document.getElementById("audio-act");
		...
		function radioValidation(){ 
			$(".act-choice-response").removeClass("act-choice-response-correct-img");
			$(".act-choice-response").removeClass("act-choice-response-incorrect-img");
			if(radioAnswer=="true"){
				$(".act-choice-response").show();
				$(".act-choice-response-txt").html($scope.multipleChoice[$scope.questionNO].response[0]);
				$(".act-choice-response").addClass("act-choice-response-correct-img"); 
				var resAudio = $scope.multipleChoice[$scope.questionNO].audio[0].response[0]; 
				playAudio(resAudio); 
				checkNextQ();
				...
				$(".act-choice-response").addClass("act-choice-response-correct-img"); 
				var resAudio = $scope.multipleChoice[$scope.questionNO].audio[0].response[1]; 
				playAudio(resAudio); 
			}else{
				$(".act-choice-response").show();
				$(".act-choice-show-answer").show(); 
				$(".act-choice-response-txt").html($scope.multipleChoice[$scope.questionNO].response[1]);
				$(".act-choice-response").addClass("act-choice-response-incorrect-img"); 
				var resAudio = $scope.multipleChoice[$scope.questionNO].audio[0].response[1];
				playAudio(resAudio); 
			}
			checkEndAudio();
		...
		function nextButtonClick(){
			...
			questionType = $scope.multipleChoice[$scope.questionNO].type;
			checkQuestionType();
			var qstAudio = $scope.multipleChoice[$scope.questionNO].audio[0].qst;
			playAudio(qstAudio); 
			$scope.isDisabled = false;
		}
});
